...
\mathrm{HalRate}  &= \frac{\sum_{k=1}^{N}\bigl|\{\,r_j\in R_k:\hat t_j\notin V\,\}\bigr|}
  ...
  \label{eq:halrate}
\end{align}
where $\mathbf{1}[\cdot]$ is the indicator function. The two success rates are per-scenario indicator
averages---detection-success~\eqref{eq:detrate} counts a scenario as solved when the arm flags any true
attack event, and full-chain~\eqref{eq:fullrate} only when every ground-truth stage is matched---while
the hallucination rate~\eqref{eq:halrate} is micro-averaged over all proposed stages. The
technique-exact and tactic accuracies are the analogous per-event fractions of attack events assigned,
respectively, the correct technique or a technique sharing its ATT\&CK tactic.
