...
Finally, the model frames the experiments as a measurement of $(p_d,p_a)$. Substituting the regime a
current mid-sized open model occupies---strong detection but weak attribution, $p_d\approx0.9$,
$p_a\approx0.06$, with $\rho$ small---\eqref{eq:idev}--\eqref{eq:idrate} predict technique recall
$\approx0.05$--$0.08$, $\mathrm{DetRate}\approx1$, and $\mathrm{FullRate}\approx0$. This is exactly the
pattern Section~\ref{sec:results} reports, so the experiments can be read as locating each arm on the
$(p_d,p_a)$ plane: today's systems sit in the high-$p_d$, low-$p_a$ corner, and the analysis says the
distance still to travel toward the ideal is almost entirely in attribution $p_a$, not detection.
