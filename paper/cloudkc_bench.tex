...
labeled datasets, and without admitting that the model's answers can change from run to run. On top
...
Prior work on turning telemetry into detections falls into four families. The first is \emph{rule-based
detection}, exemplified by the vendor-neutral Sigma standard~\cite{sigma}, whose strengths are
transparency and low cost but whose weakness is a high false-positive rate, because generic rules
match the ordinary administrative actions that attackers also use. The second is \emph{provenance- and
graph-based detection}, which links log entries into a causal graph so a multi-stage attack appears as
a connected path; a dedicated survey documents its maturity~\cite{provsurvey} and recent work applies
transformer models over provenance graphs~\cite{tbdetector}, but these methods were designed for host
logs rather than cloud control-plane telemetry and assume a causal signal that cloud API logs do not
provide. The third is \emph{LLM-assisted tooling}, closest to ours, which has used language models to
...
from live telemetry, and are rarely evaluated against fair baselines. The fourth is \emph{security
benchmarks}~\cite{cybersoceval, excytin, orgforge, acseeval, mukherjee}, which advance reproducibility
but target a different platform or task. Related attack-emulation tools such as Stratus Red
Team~\cite{stratus} and CloudGoat~\cite{cloudgoat} provide ATT\&CK-mapped cloud scenarios, but they are
generators of adversary activity rather than scored benchmarks with ground-truth answers. Taken
together, no existing approach reconstructs AWS control-plane kill chains from live telemetry with
automatic, mechanical scoring.
...
without acknowledging model variability, making headline numbers hard to trust.
...
Technique-level metrics use parent-technique credit unless stated; event-level metrics ignore the
technique label. For the two pre-registered hypotheses we report bootstrap 95\% confidence intervals,
Cohen's $d_z$, and Holm--Bonferroni-corrected permutation $p$-values; other comparisons are
descriptive.
...
Fig.~\ref{fig:evprec} plots each arm's event recall against its event precision. The three
language-model arms sit high (A3 at precision 1.000), while the rule-based arm sits to the lower
right---high recall but the lowest precision. For a security operations center, where analyst time is
scarce, the model's high precision is the more valuable property.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{event_precision_recall.pdf}
\caption{Event-level recall vs.\ precision per arm (multi-stage, 32B). The LLM arms are more precise
than the rules.}
\label{fig:evprec}
\end{figure}
...
\ref{fig:scaleev}, and~\ref{fig:scalecat} isolate model size. A validity check appears first: the
...
0.080---but stays an order of magnitude below the rules. Fig.~\ref{fig:scaleev} is more striking: at
the event level the two models are almost indistinguishable, with even the 7B model at $\sim$0.90
recall and $\sim$0.90 precision. The model's useful capability, locating attack events, does not
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_model_scale_event.pdf}
\caption{Event-level recall and precision, 7B vs.\ 32B (multi-stage): detection is scale-robust.}
\label{fig:scaleev}
\end{figure}
...
the model names distinctive techniques reliably (brute-force credential attacks every time) but for
techniques whose footprint resembles a sibling it consistently picks the wrong family member---network
discovery is labeled as instance creation, and account manipulation is mapped onto neighboring
privilege-escalation techniques. Fig.~\ref{fig:confscale} asks whether a larger model repairs this,
and the answer is largely no: scale wins on the distinctive techniques it already handles but changes
nothing on the many both models miss. The attribution bottleneck is structural, rooted in the
ambiguity of the telemetry, not dissolved by a bigger model.
...
To characterize detection quality beyond recall and precision, we recover the full event-level
confusion matrix per arm---true and false positives and negatives---and report a standard set of
classification metrics, micro-averaged over events, in Table~\ref{tab:extended}. These metrics matter
because the event-detection task is highly imbalanced (attack events are a small minority), a regime in
which raw accuracy is misleading and measures such as the Matthews Correlation Coefficient (MCC),
balanced accuracy, and the false-positive rate (FPR) are more informative.

The extended metrics sharpen the central finding rather than soften it. On the MCC---a single balanced
score for imbalanced detection---the raw language-model arm reaches $0.745$ against the rules' $0.393$,
nearly double, and the same arm records an FPR of $0.000$ against the rules' $0.750$: the rules flag
three of every four non-attack events, whereas the model flags essentially none. Balanced accuracy
($0.889$ vs $0.620$) and specificity ($1.000$ vs $0.250$) tell the same story. Figure~\ref{fig:radar}
visualizes this as a multi-metric profile: the language-model arms trace large, well-rounded polygons,
while the rule-based arm is spiky---strong on recall but collapsing on specificity, balanced accuracy,
and MCC. Figure~\ref{fig:roc} places each arm as an operating point in ROC space, where the
language-model arms sit near the ideal top-left corner and the rules sit far to the right; the compact
heatmap in Figure~\ref{fig:heat} summarizes all metrics at once.
...
deterministic rules), and Figure~\ref{fig:stabox} shows the full per-run distributions are tight. This
turns the usual hand-wave about model variability into a measured, and reassuringly small, quantity.
Finally, latency is reported as percentiles rather than a mean: the multi-agent arm has a median of
about $13.2$ seconds and a 95th percentile of $17.0$ seconds per scenario, roughly double the
single-agent arm (Figure~\ref{fig:latbox}), reinforcing that its extra cost buys no accuracy.
Efficiency, measured as event recall per thousand tokens, is highest for the single-agent arm at
$0.58$, consistent with it being the most economical language-model configuration.

\begin{table}[t]
\caption{Extended event-level metrics (Qwen2.5-32B, multi-stage, micro-averaged over events). FPR:
...
\toprule
Arm & Rec. & Prec. & $F_1$ & FPR & Spec. & Bal.Acc. & MCC \\
\midrule
A1 & 0.938 & 0.889 & 0.913 & 0.211 & 0.789 & 0.864 & 0.746 \\
A2 & 0.756 & 0.968 & 0.849 & 0.044 & 0.956 & 0.856 & 0.682 \\
A3 & 0.778 & \textbf{1.000} & 0.875 & \textbf{0.000} & \textbf{1.000} & \textbf{0.889} & \textbf{0.745} \\
A4 (rules) & \textbf{0.991} & 0.704 & 0.823 & 0.750 & 0.250 & 0.620 & 0.393 \\
\bottomrule
\end{tabular}
\end{table}
...
Several limitations qualify these conclusions. The scenarios are authored, so realism has a ceiling,
mitigated by real-incident grounding, a sealed hold-out set, and real-AWS execution. The rules'
near-optimality is partly a construct artifact, which is why we report both lenses. Model
non-determinism threatens reproducibility, addressed by a pinned model and three seeds. Per-category
samples are small, so confirmatory claims are limited to two pre-registered tests. An enabled managed
cloud detector produced no findings mapping to the emulated techniques, because our executed-API
telemetry lacks the network and threat-intelligence signals such a detector consumes; we report this
as an honest null and retain the community rule set as the scored external baseline. The study uses
one sandbox account and one model family, so the scaling result is indicative rather than definitive.

% ====================================================================
\section{Conclusion}\label{sec:conc}
We introduced CloudKC-Bench, the first open, ground-truthed benchmark for rebuilding AWS control-plane
multi-stage kill chains, and used it to run a pre-registered, controlled four-arm comparison, with an
independent rule-based baseline, on real AWS data. The central result is a decomposition: a mid-sized
open language model detects cross-domain attack events at higher precision than deterministic rules,
yet fails at exact technique attribution, with systematic confusions among related techniques. Both
pre-registered hypotheses resolved as negatives, and scaling from seven to thirty-two billion
parameters improved attribution only slightly while leaving detection essentially unchanged. The
durable contribution is the benchmark itself: an open, mechanically scored artifact that turns
unfalsifiable claims into measurable ones. Future work will close the signal gap that made both the
rules near-optimal and a managed detector silent, by raising scenario realism toward noisy production
telemetry, and will extend to more capable models, passive network telemetry, the sealed hold-out
evaluation, and adversarial-evasion scenarios.
