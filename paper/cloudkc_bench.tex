...
from the single agent on F1 by more than a small equivalence band. An a-priori power analysis (paired
...
$0.250$) tell the same story. The heatmap in Figure~\ref{fig:heat} shows every metric at once: the
...
deterministic rules), and Figure~\ref{fig:stabox} shows the per-run distributions are tight, which
...
Because both models ran over identical events, Table~\ref{tab:scale} and Figure~\ref{fig:scale_fig}
