%% CloudKC-Bench — full paper (maam-spec structure), IEEEtran journal.
...
\author{Atishay~Jain, Annamalai~N, and S.~Nagasundari%
\thanks{The authors are with the Centre for Information Security, Forensics and Cyber Resilience
(ISFCR), PES University, Bengaluru, India (e-mail: atishay.offical@gmail.com).}}
...
Cloud computing is now the default way organizations run their software. Gartner expects worldwide
spending on public cloud services to reach US\$723.4 billion in
...
This is why researchers have begun to ask whether large language models (LLMs) can help. Recent surveys
describe a fast-growing body of work that applies them to reading logs, spotting anomalies, checking for
vulnerabilities, and assisting analysts~\cite{llmsurvey1, llmsurvey2, llmsurvey3}. The appeal is obvious.
A language model can read messy, mixed data and reason about it in plain language, and it might replace
the rigid hand-written rules that most security tools still rely on. Some cloud-focused systems already
do this: writing detection rules from threat reports~\cite{llmcloudhunter}, or adding a model to explain
the alerts of a conventional detector~\cite{huntgpt}.

Reconstructing a multi-step cloud attack is a much harder problem than labeling a single event, and it
is exactly where the value of LLM correlation is least understood. The kill-chain idea, introduced by
Hutchins \emph{et al.}~\cite{hutchins} and made practical for the cloud by the MITRE ATT\&CK
framework~\cite{mitre}, describes an attack as an ordered set of stages: getting in, gaining privileges,
evading defenses, stealing credentials, exploring, moving sideways, stealing data, and causing damage.
In the cloud each stage tends to surface in a different log. Administrative actions land in CloudTrail,
network traffic in VPC Flow Logs, file operations in storage logs, and machine events in compute logs.
None of these logs shares a common key, and they arrive at different and unpredictable times, so
reassembling the chain means connecting clues \emph{across} logs rather than within any single
one~\cite{wilkens, hetfusion}.

Yet the claims in this area are often made without the safeguards that would let anyone check them.
Results are reported without fair baselines that separate the model's contribution from the rest of the
system, without shared and clearly labeled datasets, and without acknowledging that a model's answers
can change from run to run. These are exactly the pitfalls shown to inflate results across security
machine learning~\cite{arp22}. Compounding the problem, no public benchmark exists for rebuilding AWS
control-plane kill chains with answers a computer can check automatically. The benchmarks that do exist
target a different platform or a different task (malware analysis, threat-intelligence question
answering, architecture modeling, insider threat), or they measure how well a system \emph{writes}
detection rules rather than how well it \emph{rebuilds} an attack~\cite{cybersoceval, excytin, orgforge,
acseeval}. The community is left without the tool to measure whether LLM correlation actually helps, and
without the method to say which part of a system deserves the credit.

This paper addresses that gap. We present CloudKC-Bench, an open, answer-checked benchmark for
rebuilding AWS control-plane multi-stage kill chains, and use it to run a pre-registered, controlled
comparison of four system variants (plus an independent rule-based baseline) on real AWS data,
reporting the results whether or not the language model wins. The principal contributions of
...
implementation. Section~\ref{sec:expected} derives the benchmark's expected behavior under an idealized
capability model. Section~\ref{sec:results} reports the experimental results. Section~\ref{sec:disc}
...
Three ideas recur throughout this paper: the data a cloud platform produces, the description of attacks
as staged sequences, and the recent use of language models to interpret security data.

A cloud account constantly produces \emph{telemetry}, the records of what happened to its resources. On
Amazon Web Services four sources carry most of the security signal. CloudTrail records administrative
API calls, VPC Flow Logs record network connections, storage access logs record object operations, and
compute logs record when machines start or stop. Each source captures only one slice of behavior, and no
single source contains a whole attack. Attacks, in turn, are usually described with the \emph{kill
chain}~\cite{hutchins}; the MITRE ATT\&CK framework~\cite{mitre} turns that idea into a catalog of
concrete \emph{techniques}, each mapped to a tactical goal. Language models have recently joined this
picture, with broad surveys reporting fast-growing work on log analysis, anomaly detection, and analyst
support~\cite{llmsurvey1, llmsurvey2, llmsurvey3}.

Individually these concepts are mature. What remains largely unstudied is their combination: using a
model to rebuild an ATT\&CK-mapped kill chain from several disconnected cloud logs at once, and whether
doing so measurably helps.
...
We state the benchmark and the study it enables in one formal frame; the constructions of the following
subsections are instances of its components. The benchmark is the tuple
\begin{equation}
\mathcal{B} = (\mathcal{Z},\ \pi,\ \mu,\ \Phi),
\end{equation}
where $\mathcal{Z}=\{z_k\}_{k=1}^{N}$ is a set of scenarios, each $z_k=(T_k,y_k)$ pairing observable
telemetry $T_k$ with a hidden manifest $y_k=(M_k,G_k)$ that records the ground-truth stages $M_k$ and the
true attack events $G_k$. The map $\pi$ is the \emph{visibility projection}: it removes every technique
label and attack flag from the telemetry, so $\pi(T_k)$ is exactly what a detection system is permitted
to read. A detection \emph{arm} is then a function
\begin{equation}
A:\ \pi(\mathcal{T})\ \to\ \mathcal{R},\qquad R_k=A\bigl(\pi(T_k)\bigr),
\end{equation}
mapping visible telemetry to a reconstructed chain $R_k$. This turns the answer-key boundary into a
formal constraint: because $A$ receives only $\pi(T_k)$, its output is independent of the hidden manifest,
\begin{equation}
R_k \perp y_k,
\end{equation}
so no arm can condition on the labels it is meant to predict. The matching function $\mu$
(Eq.~\ref{eq:match}) decides when a reported stage agrees with a ground-truth stage, and the scorer
$\Phi(R,y)$ aggregates its outcome into the metric vector reported throughout: technique-level
precision, recall, and $F_1$; the event-level pair; and the task-specific reliability rates.

Given these components, evaluating an arm on category $c$ averages the score over that category's
scenarios, with the $S$ stochastic seeds averaged per scenario before aggregation and categories never
pooled:
\begin{equation}
\mathrm{Eval}_c(A)=\frac{1}{|\mathcal{Z}_c|}\sum_{k\in\mathcal{Z}_c}\frac{1}{S}\sum_{s=1}^{S}
\Phi\bigl(A^{(s)}(\pi(T_k)),\,y_k\bigr).
\label{eq:eval}
\end{equation}
Because the four arms share one $\mathcal{Z}$ and one $\pi$, their scores can differ only through $A$, so
the controlled ablation reduces to a difference of functionals,
\begin{equation}
\Delta_c(A_i,A_j)=\mathrm{Eval}_c(A_i)-\mathrm{Eval}_c(A_j).
\label{eq:delta}
\end{equation}
The two pre-registered hypotheses are particular instances on the multi-stage category: H1 tests
$\Delta(A_1,A_4)$ on technique-level recall, and H2 tests $\Delta(A_1,A_2)$ on $F_1$.
...
flagged, then
\begin{align}
\mathrm{event\_recall}&=\frac{|P\cap G|}{|G|},\\
\mathrm{event\_precision}&=\frac{|P\cap G|}{|P|}.
\end{align}
Reporting both separates \emph{finding} an attack from \emph{labeling} it.

\subsection{Pre-Registration and Statistical Protocol}
To keep the study honest, the research question, the hypotheses, and every statistical threshold were
fixed in writing \emph{before any result was computed}. Two confirmatory hypotheses are pre-registered
on the multi-stage category, the primary test bed: H1 asks whether the best language-model arm exceeds
the rules on technique-level recall by a pre-set margin, and H2 asks whether the multi-agent arm differs
from the single agent on $F_1$ by more than a small equivalence band. An a priori power analysis (paired
design, $\alpha=0.05$, power $0.80$) showed that, after correcting for multiple comparisons across the
full grid of arms and categories, only very large effects are detectable at the frozen sample size; we
therefore pre-registered exactly \emph{two} confirmatory tests and treat every other comparison as
exploratory, reported with effect sizes and confidence intervals rather than as significance claims.

The unit of analysis is the scenario, never the individual event or seed: the three seeds are repeated
measures averaged to a single value per scenario before any aggregation, and the five categories are
never pooled. For each reported quantity we give a bootstrap $95\%$ confidence interval (percentile
method over resampled scenarios), and for the two confirmatory tests we add a paired effect size
(Cohen's $d_z$) and a paired permutation $p$-value, corrected across the two tests with the
Holm--Bonferroni procedure. This protocol is what lets us report the negative results (H1 unsupported,
H2 not rejected) as findings rather than as failures to reach significance.
...
% ====================================================================
\section{Expected Performance: An Idealized Analysis}\label{sec:expected}
Before turning to measured systems, we ask what CloudKC-Bench \emph{should} return as a function of a
detector's latent capability. This serves two ends. It establishes that the instrument is
well-posed (a perfect reconstructor scores perfectly, and a stronger detector never scores lower), and
it supplies a yardstick against which the numbers of Section~\ref{sec:results} can be read as an estimate
of where a real system sits.

\subsection{A Parametric Capability Model}
We summarize any detector by a few capability parameters, holding the scenario fixed. For a given true
attack event, let $p_d\in[0,1]$ be the probability the detector flags it, its \emph{detection
sensitivity}. For a stage it has already detected, let $p_a\in[0,1]$ be the probability it assigns the
correct ATT\&CK technique, its \emph{attribution accuracy}. For a benign event, let $\rho\in[0,1]$ be
the probability it is wrongly flagged, and let $\eta\in[0,1]$ be the probability that a proposed label
falls outside the closed vocabulary $V$. Events and stages are treated as detected and attributed
independently; this is the standard first-order simplification and is exact for the expectations below.

Fix a scenario $z_k$ with $n_k$ stages, $g_k=|G_k|$ true attack events, and $b_k$ benign events.
Detecting a stage's evidence event supplies its source automatically, since each event carries its own
$\sigma$, so under the matching test~\eqref{eq:match} a stage is a technique-level true positive exactly
when its evidence is detected \emph{and} its technique is named correctly, an event of probability
$p_d\,p_a$.

\subsection{Expected Metrics in Closed Form}
Taking expectations over the detector's randomness gives every reported quantity as a function of these
parameters:
\begin{align}
\mathbb{E}[\mathrm{ev\_recall}] &= p_d, \label{eq:idev}\\[2pt]
\mathbb{E}[\mathrm{ev\_prec}] &= \frac{p_d\,g_k}{p_d\,g_k+\rho\,b_k}, \label{eq:idprec}\\[2pt]
\mathbb{E}[\mathrm{tech\_recall}] &= p_d\,p_a, \label{eq:idtechrec}\\[2pt]
\frac{\mathbb{E}[\mathrm{tech\_recall}]}{\mathbb{E}[\mathrm{ev\_recall}]} &= p_a, \label{eq:idtech}\\[2pt]
\mathbb{E}[\mathrm{DetRate}] &= 1-(1-p_d)^{g_k}, \label{eq:iddet}\\[2pt]
\mathbb{E}[\mathrm{FullRate}] &= (p_d\,p_a)^{n_k}. \label{eq:idrate}
\end{align}
with false-positive rate $\rho$, specificity $1-\rho$, hallucination rate $\eta$, and balanced accuracy
$\tfrac12\bigl(p_d+(1-\rho)\bigr)$. Three features of these expressions drive everything that follows.
First, the two lenses \emph{identify} the two capabilities separately: the event lens estimates $p_d$,
and the technique-to-event ratio~\eqref{eq:idtech} estimates the attribution accuracy $p_a$ on its own.
This is the formal justification for reporting both. Second, full-chain success decays as
$(p_d p_a)^{n_k}$, exponentially in chain length, so even a strong per-stage reconstructor collapses on
long chains. Third, the detection-success rate saturates: for $g_k$ of even moderate size,
$1-(1-p_d)^{g_k}\approx 1$ unless $p_d$ is tiny, which is why we treat DetRate as a floor rather than a
discriminating metric.

\subsection{Well-Posedness and the Ideal Limit}
\noindent\textbf{Proposition (monotonicity and saturation).} \emph{For any fixed scenario structure,
each expected metric in~\eqref{eq:idev}--\eqref{eq:idrate} is non-decreasing in $p_d$ and $p_a$ and
non-increasing in $\rho$ and $\eta$, and attains its optimum (unity for the recalls, precisions, $F_1$,
DetRate, FullRate, and balanced accuracy, and zero for the false-positive and hallucination rates) exactly
at the ideal point $(p_d,p_a,\rho,\eta)=(1,1,0,0)$.}

The proof is immediate, as each expression is a monotone function of the parameters. The consequence is
what makes CloudKC-Bench a valid instrument: no score can be maximized except by a detector that both
finds every attack event and names every technique, and any genuine capability gain moves the score in
the right direction. In the ideal limit an oracle LLM attains event and technique recall $1$, precision
$1$, FullRate $1$, DetRate $1$, and both the false-positive and hallucination rates $0$. The benchmark's
ceiling is exactly a perfect kill-chain reconstruction, as intended.

\subsection{What a Strong Model Should Achieve}
The same closed form predicts a strong-but-imperfect near-future model and sets concrete targets. A
capable detector might reach $p_d=0.95$ and $p_a=0.90$ at $\rho=0.02$; then~\eqref{eq:idev}--\eqref{eq:idrate}
give event recall $0.95$, technique recall $0.855$, and, because full-chain success is
exponential, $\mathrm{FullRate}=(0.855)^{n}$, that is $0.53$ for a four-stage chain and $0.39$ for six.
Requiring $\mathrm{FullRate}\ge\theta$ on chains of length $n$ imposes a per-stage bar
\begin{equation}
p_d\,p_a \;\ge\; \theta^{1/n}, \label{eq:threshold}
\end{equation}
so even a coin-flip whole-chain rate ($\theta=0.5$) on four-stage chains already demands
$p_d\,p_a\ge 0.84$. Only near-perfect per-stage reconstruction yields reliable whole-chain recovery,
which is precisely why full-chain success is the benchmark's hardest bar.

Finally, the model frames the experiments as a measurement of $(p_d,p_a)$, as Fig.~\ref{fig:capplane}
makes visual. Substituting the regime a current mid-sized open model occupies (strong detection but weak
attribution, $p_d\approx0.9$, $p_a\approx0.06$, with $\rho$ small), \eqref{eq:idev}--\eqref{eq:idrate}
predict technique recall $\approx0.05$--$0.08$, $\mathrm{DetRate}\approx1$, and
$\mathrm{FullRate}\approx0$. This is exactly the pattern Section~\ref{sec:results} reports, so the
experiments can be read as locating each arm on the $(p_d,p_a)$ plane: today's systems sit in the
high-$p_d$, low-$p_a$ corner, and the analysis says the distance still to travel toward the ideal is
almost entirely in attribution $p_a$, not detection.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_capability_plane.pdf}
\caption{The capability plane. Technique recall equals $p_d\,p_a$ (shaded contours); the event lens
measures $p_d$ and the technique-to-event ratio measures $p_a$. A perfect reconstructor sits at the ideal
corner $(1,1)$; a strong near-future model near $(0.95,0.90)$; today's mid-sized open models in the
high-$p_d$, low-$p_a$ corner. The remaining path to the ideal is almost vertical: gains must come from
...
One caveat governs how the strict-lens numbers should be read, and we raise it here rather than bury it.
The rules reach $0.867$ technique recall because their signatures effectively invert our scenario
generator, recovering the exact sub-technique each scenario was built to encode
(Section~\ref{sec:disc}). The strict lens therefore flatters any method that has encoded the answer key,
so we treat the event-level lens as the primary comparison throughout.
...
The richer classification metrics defined above, reported in Table~\ref{tab:extended}, reinforce this
finding. On the MCC (the single balanced score for imbalanced detection) the raw
...
$0.250$) tell the same story. The heatmap in Fig.~\ref{fig:heat} shows every metric at once: the
language-model arms hold up across the board, while the rule-based arm is strong on recall but collapses
on specificity, balanced accuracy, and MCC.
...
deterministic rules), and Fig.~\ref{fig:stabox} shows the per-run distributions are tight, which
replaces the usual hand-waving about model variability with a small, measured value. Latency is
...
(Fig.~\ref{fig:latbox}), while event recall per thousand tokens is highest for the single-agent arm at
...
Figure~\ref{fig:percat_fig} and Table~\ref{tab:percat} show the pattern is not peculiar
...
\begin{figure*}[t]
\centering
\subfloat[Technique-level recall.]{\includegraphics[width=0.49\textwidth]{recall_by_category.pdf}\label{fig:reccat}}
\hfil
\subfloat[Event-level recall.]{\includegraphics[width=0.49\textwidth]{percategory_event.pdf}\label{fig:evcat}}
\caption{Recall by arm across the five categories (32B, real-AWS): (a) technique-level and (b)
event-level. The rules dominate technique-level recall everywhere; the model arms recover most true
attack events at the event level.}
\label{fig:percat_fig}
\end{figure*}
...
\emph{not supported}. The multi-agent arm differs from the single-agent arm by only $+0.027$ in $F_1$
...
Because both models ran over identical events, Table~\ref{tab:scale} and Fig.~\ref{fig:scale_fig}
isolate model size. A validity check appears first: the
...
\begin{figure}[t]
\centering
\subfloat[Per arm, multi-stage.]{\includegraphics[width=0.9\linewidth]{fig_model_scale_multistage.pdf}\label{fig:scalems}}\\
\subfloat[By category, arm A1.]{\includegraphics[width=0.9\linewidth]{fig_model_scale_percategory.pdf}\label{fig:scalecat}}
\caption{Technique-level recall, 7B vs.\ 32B on identical real-AWS telemetry: (a) per arm on the
multi-stage category and (b) by category for arm A1. Scaling helps naming only slightly.}
\label{fig:scale_fig}
\end{figure}
...
We define these rates formally over the $N$ scenarios of a category. For scenario $k$, let $M_k$ be its
ground-truth stages ($n_k=|M_k|$) and $G_k$ its true attack events; let the arm report the stage set
$R_k$, each $r_j$ carrying a technique label $\hat t_j$, flag the event set $P_k$, and match
$\mathrm{TP}_k$ stages under the function of Section~\ref{sec:framework}. Writing $V$ for the closed
technique vocabulary supplied in the prompt,
\begin{align}
\mathrm{DetRate}  &= \frac{1}{N}\sum_{k=1}^{N}\mathbf{1}\!\left[\,P_k\cap G_k\neq\varnothing\,\right],
  \label{eq:detrate}\\[2pt]
\mathrm{FullRate} &= \frac{1}{N}\sum_{k=1}^{N}\mathbf{1}\!\left[\,\mathrm{TP}_k=n_k\,\right],
  \label{eq:fullrate}\\[2pt]
\mathrm{HalRate}  &= \frac{\sum_{k=1}^{N}\bigl|\{\,r_j\in R_k:\hat t_j\notin V\,\}\bigr|}
  {\sum_{k=1}^{N}\lvert R_k\rvert},
  \label{eq:halrate}
\end{align}
where $\mathbf{1}[\cdot]$ is the indicator function. The two success rates bracket the difficulty of a
scenario from opposite ends. The detection-success rate~\eqref{eq:detrate} is the most lenient: a
scenario counts as solved the moment the arm flags even one of its true attack events, so it asks only
whether the attack was noticed at all. The full-chain success rate~\eqref{eq:fullrate} is its strict
complement, crediting a scenario only when \emph{every} ground-truth stage is matched, the complete
reconstruction an analyst ultimately needs. The distance between the two is itself informative: a high
detection rate beside a low full-chain rate means an arm reliably senses that something is wrong but
rarely rebuilds the whole chain, which is exactly the pattern we observe for the language model.

The hallucination rate~\eqref{eq:halrate} measures faithfulness rather than coverage. It is the fraction
of proposed stages whose technique label falls outside the closed vocabulary $V$ that the prompt
supplies, so a non-zero value means the model named an identifier it was explicitly told not to use. We
micro-average over all proposals rather than over scenarios, so an arm that emits many spurious labels is
penalized in proportion to how often it does so; lower is better. Read together, the three rates form a
floor (did the arm notice the attack?), a ceiling (did it reconstruct every stage?), and a reliability
check (did it stay inside the allowed label set?). The technique-exact and tactic accuracies complete the
picture as the analogous per-event fractions of attack events assigned, respectively, the correct
technique or a technique sharing its ATT\&CK tactic.
...
\subsection{Ordering Robustness to Delivery Lag}
Because a kill chain is an \emph{ordered} sequence, we test how sensitive the recovered ordering is to
the cross-source delivery lags that characterize real cloud telemetry, where administrative logs arrive
minutes late while network and compute logs arrive within seconds. For each multi-stage scenario we take
the ground-truth candidates, shift the timestamps of one telemetry source by an offset $\Delta T$, and
re-run the correlator, sweeping $\Delta T$ from $-900$ to $+900$ seconds. Figure~\ref{fig:tcp} reports
the outcome. Detection recall is completely invariant to $\Delta T$ (the matching function keys on
techniques and evidence, not timestamps), so any change is a pure \emph{ordering} effect. The
order-aware recall stays perfect within a robust band of about $\pm 120$ seconds and then degrades,
falling to $0.84$ at the extremes. The practical implication matters: because the gap between
CloudTrail's delivery lag ($\sim$7 minutes) and a compute log's ($\sim$30 seconds) far exceeds $120$
seconds, realistic delivery-lag skew can misorder stages even when every event is detected. A measured
clock model and an explicit correlation window are therefore necessary components of a faithful
reconstruction system, not optional refinements.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_tcp_robustness.pdf}
\caption{$\Delta T$ sensitivity on multi-stage scenarios. Detection recall (green) is invariant to a
per-source timestamp offset; order-aware recall (blue) is perfect only within a $\pm 120$\,s band and
degrades beyond it, isolating an ordering failure from a detection failure.}
\label{fig:tcp}
\end{figure}

% ====================================================================
\section{Discussion}\label{sec:disc}
This study does not crown a winner; it separates two capabilities that are usually treated as one. On
real AWS multi-stage kill chains, a mid-sized open language model detects cross-domain attack events at
higher precision than deterministic rules but fails at exact technique attribution. For practitioners
the implication is that the two capabilities belong to different tools: the model is well suited to
high-precision triage, and exact labeling is better left to rules or a human analyst. The model's
precision advantage has a simple cause. It reasons about an event in context and can decline to flag a
lone action that a rule would trip on, which is also why removing the pre-filter improves its recall.

One limitation shapes how the strict-metric results should be read, and we state it plainly. The
rule-based arm dominates technique-level recall not because rules are inherently better attributors but
because its signatures are effectively the inverse of the scenario generator: they recover the exact
sub-technique each scenario was built to encode. Executing against real AWS does not dissolve this
advantage, because the captured telemetry is still the structured API calls the benchmark issues. The
strict metric thus rewards any method that has encoded the answer key, and cannot by itself settle
whether a language model or a rule set is the better attributor. This is why we lead with the
event-level lens, and why the decisive test is left for future work: it needs noisy production telemetry
in which no static rule can invert the data.

Read with that caveat, three practical points hold. Multi-agent decomposition is not worth its cost.
The pre-filter is a tunable cost-versus-recall knob rather than a free optimization. And because
detection barely moves with model size, a deployment that needs only triage can run a small model.
...
\emph{Construct validity} asks whether the metrics measure what we intend. As discussed above, the
rule-based arm's near-optimal technique-level recall is partly an artifact of the benchmark, because its
signatures invert the scenario generator. We address this by reporting two complementary lenses, leading
with event-level detection, and including an independent community rule baseline that is not tuned to our
scenarios. The bespoke matching function is a further construct risk; we mitigate it by leading with
standard classification metrics (Section~\ref{sec:results}) and by publishing the function for
...
average over three seeds, and report the measured run-to-run variance, which is small. Because every arm
sees identical model, telemetry, and compute, any score difference is attributable to the one factor
under test.

\emph{Statistical conclusion validity} turns on whether our inferences are sound given the sample size.
Per-category sample sizes are small, so an a priori power analysis restricts confirmatory claims to the two
...
network- and threat-intelligence-level signals such a detector consumes. We therefore report it as a
null result and retain the community rule set as the scored external baseline; raising scenario realism
...
long-term reproducibility; we mitigate them by pinning model versions and releasing the complete
artifact described in the availability statement. Adversarial evasion of the language model is out of
...
already in use? By fixing the model, telemetry, and compute budget and varying one factor at a time,
the benchmark makes that question measurable instead of rhetorical.

The central result separates two capabilities rather than declaring a winner. A mid-sized open language
model detects cross-domain attack events broadly and at higher precision than deterministic rules (on the
...
enabled managed detector produced no relevant findings, a null we report rather than omit. For
...
Beyond any single result, the lasting contribution is the benchmark itself: an open, mechanically
scored artifact that turns previously unfalsifiable claims about language models in cloud security into
measurable ones, and that should remain useful as models improve. Two threads of future work follow
...
benchmark and its mixed results encourage the community to measure, rather than assume, what language
models contribute to reconstructing cloud attacks.

% ====================================================================
\section*{Reproducibility and Availability}
CloudKC-Bench is released as a self-contained artifact comprising the scenario generator, the
machine-checkable manifests and their JSON schema, the deterministic matching function, the four arms
and the Sigma baseline, and every analysis and figure-generation script used in this paper. The
synthetic backend regenerates identical telemetry offline, without a cloud account or a graphics card,
so the reproducibility layer of every result can be reproduced on a commodity laptop; the real-AWS
results are reproduced by replaying the released scenarios against a sandbox account. Experiments pin a
single model version, record it with every result, and warn when a database mixes models; a
continuous-integration suite exercises the full pipeline on three Python versions on every change. Each
model or environment is stored in its own database to prevent accidental mixing, and the two-phase run
separates the billable capture step from the offline scoring step so that analyses can be repeated
without re-incurring cloud cost.
...
\bibitem{gartner99} Gartner, ``Is the Cloud Secure?,'' Oct.\ 2019. [Online]. Available: gartner.com/smarterwithgartner/is-the-cloud-secure
...
\bibitem{llmsurvey1} S. Tian \emph{et al.}, ``Exploring the Role of Large Language Models in Cybersecurity: A Systematic Survey,'' arXiv:2504.15622, 2025.
\bibitem{llmsurvey2} N. O. Jaffal, M. Alkhanafseh, and D. Mohaisen, ``Large Language Models in Cybersecurity: A Survey of Applications, Vulnerabilities, and Defense Techniques,'' \emph{AI} (MDPI), vol.\ 6, no.\ 9, art.\ 216, 2025.
\bibitem{llmsurvey3} S. Akhtar, S. Khan, and S. Parkinson, ``LLM-based Event Log Analysis Techniques: A Survey,'' arXiv:2502.00677, 2025.
...
\bibitem{hetfusion} X. Zang, J. Gong, X. Zhang, and G. Li, ``Attack Scenario Reconstruction via Fusing Heterogeneous Threat Intelligence,'' \emph{Computers \& Security}, vol.\ 133, art.\ 103420, Elsevier, 2023.
\bibitem{cybersoceval} L. Deason \emph{et al.}, ``CyberSOCEval: Benchmarking LLMs for Malware Analysis and Threat Intelligence Reasoning,'' arXiv:2509.20166, 2025.
\bibitem{excytin} Y. Wu \emph{et al.}, ``ExCyTIn-Bench: Benchmarking LLM Agents on Cyber Threat Investigation,'' arXiv:2507.14201, 2025.
\bibitem{orgforge} J. Flynt, ``OrgForge-IT: A Verifiable Synthetic Benchmark for LLM-Based Insider Threat Detection,'' arXiv:2603.22499, 2026.
\bibitem{acseeval} S. Munshi, S. Pathak, S. Ghatode, T. Priyadarshini, D. Chandramouleeswaran, and A. Rana, ``ACSE-Eval: Can LLMs Threat Model Real-World Cloud Infrastructure?,'' arXiv:2505.11565, 2025.
...
\bibitem{tbdetector} N. Wang \emph{et al.}, ``TBDetector: Transformer-Based Detector for Advanced Persistent Threats with Provenance Graph,'' arXiv:2304.02838, 2023.
...
\bibitem{aisecevent22} D. Levshun and I. Kotenko, ``A Survey on Artificial Intelligence Techniques for Security Event Correlation: Models, Challenges, and Opportunities,'' \emph{Artificial Intelligence Review}, vol.\ 56, no.\ 8, pp.\ 8547--8590, Springer, 2023.
\bibitem{maac} X. Wang, X. Gong, L. Yu, and J. Liu, ``MAAC: Novel Alert Correlation Method to Detect Multi-step Attack,'' arXiv:2011.07793, 2020.
\bibitem{graphweaver} S. Freitas and A. Gharib, ``GraphWeaver: Billion-Scale Cybersecurity Incident Correlation,'' arXiv:2406.01842, 2024.
\bibitem{secure_bench} D. Bhusal \emph{et al.}, ``SECURE: Benchmarking Large Language Models for Cybersecurity Advisory,'' arXiv:2405.20441, 2024.
\bibitem{llmbenchsurvey} S. Ni \emph{et al.}, ``A Survey on Large Language Model Benchmarks,'' arXiv:2508.15361, 2025.
\end{thebibliography}
