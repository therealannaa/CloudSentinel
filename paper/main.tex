...
signatures encode the answer key. \todo{regenerate from the final 32b / real-AWS run.}}
\label{fig:twolens}
\end{figure}
...
synthetic). A4 rules dominate the strict metric everywhere; the LLM arms are low
across the board, consistent with the attribution bottleneck.
\todo{swap in real-AWS results as the headline once available.}}
\label{fig:recallcat}
\end{figure}
...
technique-level recall, per arm (qwen2.5:32b, synthetic). A4/SIGMA sit at zero token
cost; the LLM arms pay tokens without a technique-level return on this metric.
\todo{regenerate from the final run; add the event-level panel.}}
\label{fig:costacc}
\end{figure}
...
attached ATT\&CK id is usually wrong (Fig.~\ref{fig:confusion}).
...
misses entirely. \todo{generate from the 32b run DB once uploaded.}}
\label{fig:confusion}
