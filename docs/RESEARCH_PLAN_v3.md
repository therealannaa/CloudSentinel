...
| **P3 · Arms + Baselines** | 4 arms + 3 external baselines | A1 (hunters+coordinator), A2 (single agent), A3 (raw-log), A4 (rules + **community Sigma**); **LLMCloudHunter** reimpl; GuardDuty on real AWS; pre-filter instrumentation for cost/latency (`14`). | Atishay (A1, A4, pre-filter) / Anna (collectors, A2/A3, LCH) | **[~Wk 5–9]** |
...
3. **Matching-fn spec `04` → Atishay's P3 arms** (⚠️ blocking) and the baseline adapters (`11`).
...
3. External baselines (GuardDuty, LLMCloudHunter reimpl, community Sigma) run on the full benchmark.
...
11. TCP disjoint-stream robustness test with ΔT sensitivity.
